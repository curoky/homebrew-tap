class Thriftc < Formula
  ...
  version ENV['HOMEBREW_THRIFTC_VERSION'] || "0.9.3"
  if version == "0.2.0"
  ...
  def autotools_install
    inreplace "compiler/cpp/src/generate/t_rb_generator.cc",
        ...
        "if (first) {first = false;} else {f_types_ << \", \";}" if Gem::Version.new(version) <= Gem::Version.new('0.9.1')

    inreplace "compiler/cpp/src/generate/t_java_generator.cc",
        "first ? first = false : indent(f_service_) << \"else \";",
        "if (first) {first = false;} else {indent(f_service_) << \"else \";}" if version == "0.9.1"
